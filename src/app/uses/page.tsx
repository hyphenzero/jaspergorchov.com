...
const categories = [
  {
    ...
    items: [
      ...
      },
      {
        name: 'VS Code',
        description:
          "Still installed for when I need something Zed can't do — extensions, debugging configs, or pair programming.",
      },
      ...
      {categories.map((category) => (
        <section key={category.title} className="grid grid-cols-1 gap-6 sm:grid-cols-12 sm:gap-10">
          <div className="sm:col-span-3">
            <h2 className="text-xl font-medium tracking-tight text-zinc-950 dark:text-white">{category.title}</h2>
          </div>
              ...
              {category.items.map((item) => {
                ...
                return (
                  <div key={item.name} className="flex items-center gap-6 py-3 first:pt-0 last:pb-0">
                    <div className="flex size-7 shrink-0 items-center justify-center text-zinc-500 dark:text-zinc-400">
                    ...
                    <div className="flex min-w-0 items-baseline gap-6">
                      <h3 className="text-sm font-medium text-zinc-950 dark:text-white">{item.name}</h3>
