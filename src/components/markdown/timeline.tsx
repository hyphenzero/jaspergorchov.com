    ...
    <div className={clsx('relative', className)}>
      ...
      {title ? (
        <h3 className="relative mt-3!">
          <span className="absolute inset-y-1 -left-[calc(var(--spacing)*8.125)] w-0.5 rounded-full bg-sky-500 ring-5 ring-white max-md:-left-[calc(var(--spacing)*6.125)] dark:bg-sky-400 dark:ring-zinc-950" />
          {title}
        </h3>
      ) : null}
      {children}
